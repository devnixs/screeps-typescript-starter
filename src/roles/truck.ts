...
class RoleTruck implements IRole {
  ...
  *getJobs(creep: Creep): IterableIterator<IJob | null> {
    ...
    if (
      ...
      droppedResource.amount > creep.pos.getRangeTo(droppedResource.pos.x, droppedResource.pos.y) * 5 &&
    ...
    if (storage && terminal) {
      ...
      if (
        terminal &&
        terminal.store.energy < desiredEnergyInTerminal * 0.8 &&
        storage &&
      ...
      if (
        terminal &&
        terminal.store.energy > desiredEnergyInTerminal * 1.2 &&
        storage &&
